...
exports.handler = async function (event, context) {
    ...
    const b64string = JSON.parse(event.body)["b64string"]
    const fixed_string = b64string.replaceAll('.', '+').replaceAll('_', '/').replaceAll('-', '=');

    const buf = Buffer.from(fixed_string, 'base64');
    ...
    try {
        ...
        var result : Result = {statusCode: 200, body: `https://www.arweave.net/${transaction.id}?ext=png`}
