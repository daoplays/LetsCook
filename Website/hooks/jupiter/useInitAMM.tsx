...
const useInitAMM = (launchData: LaunchData, listing: ListingData) => {
    ...
    const InitAMM = async () => {
        ...
        if (!LaunchData) {
            toast.error("Failed to get Get Launch Data");
            return;
        }

        if (!ListingData) {
            toast.error("Failed to get Get Listing Data");
            return;
        }

        const instruction = await GetInitAMMInstruction(connection, launchData, listing, wallet.publicKey);
