...
import { ComputeBudgetProgram, Connection, Keypair, PublicKey, SYSVAR_RENT_PUBKEY, Transaction, TransactionInstruction } from '@solana/web3.js'
...
import { Distribution, LaunchData, LaunchInstruction, bignum_to_num, getRecentPrioritizationFees, get_current_blockhash, send_transaction, uInt16ToLEBytes, uInt32ToLEBytes } from '../../components/Solana/state'
...
export function getPoolStateAccount(base_mint: PublicKey, quote_mint: PublicKey) {
    ...
    return PublicKey.findProgramAddressSync([Buffer.from(POOL_SEED), getAMMConfigAccount().toBytes(), token0.toBytes(), token1.toBytes()], RAYDIUM_PROGRAM)[0];
}
export function getObservationAccount(base_mint: PublicKey, quote_mint: PublicKey) {
    return PublicKey.findProgramAddressSync([Buffer.from(OBSERVATION_SEED), getPoolStateAccount(base_mint, quote_mint).toBytes()], RAYDIUM_PROGRAM)[0];
}
export function getLPMintAccount(base_mint: PublicKey, quote_mint: PublicKey) {
    return PublicKey.findProgramAddressSync([Buffer.from(POOL_LP_MINT_SEED), getPoolStateAccount(base_mint, quote_mint).toBytes()], RAYDIUM_PROGRAM)[0];
}
export function getAMMBaseAccount(base_mint: PublicKey, quote_mint: PublicKey) {
    return PublicKey.findProgramAddressSync([Buffer.from(POOL_VAULT_SEED), getPoolStateAccount(base_mint, quote_mint).toBytes(), base_mint.toBytes()], RAYDIUM_PROGRAM)[0];
}
export function getAMMQuoteAccount(base_mint: PublicKey, quote_mint: PublicKey) {
    return PublicKey.findProgramAddressSync([Buffer.from(POOL_VAULT_SEED), getPoolStateAccount(base_mint, quote_mint).toBytes(), quote_mint.toBytes()], RAYDIUM_PROGRAM)[0];
}
...
export const useCreateCP = (launch : LaunchData) => {
    ...
    let launch_data_account = PublicKey.findProgramAddressSync(
        [Buffer.from(launch.page_name), Buffer.from("Launch")],
        PROGRAM,
    )[0];
